...
export function KnowledgePanel(){
 ...
 {admin&&<><form className="knowledge-upload" onSubmit={async e=>{e.preventDefault();if(!file||busy)return;setBusy(true);setError('');try{if(file.size>65536)throw new Error('Choose a file up to 64 KB.');await api('/api/documents',{name:file.name,content:new TextDecoder('utf-8',{fatal:true}).decode(await file.arrayBuffer())});setFile(null);if(input.current)input.current.value='';setNotice('Uploaded as a draft. Open Preview & review to inspect its embedding before saving.');await load();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>
 ...
 <p>{configured?'Generating a preview sends the draft to your configured embedding provider. Nothing becomes searchable until you save it.':'Preview shows keyword sections because no embedding provider is configured.'}</p><button disabled={!file||busy}>Upload draft</button></form><OKFImport disabled={busy} onImported={load}/></>}
 ...
 <div className="knowledge-document-actions"><label htmlFor="publication-filter">Show</label><select id="publication-filter" value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All documents</option><option value="draft">Drafts</option><option value="review">Awaiting review</option><option value="published">Published</option></select></div>
 ...
 <div className="knowledge-documents">{visible.map(doc=><article key={doc.id}><div><h3>{doc.name}</h3><p>{doc.publication} · Revision {doc.revision} · {doc.status==='ready'?`${doc.chunk_count} sections · ${doc.model?'Semantic':'Keyword'}`:doc.status==='unprocessed'?'Awaiting embedding preview':doc.status==='processing'?`Legacy processing · attempt ${doc.attempts}/3`:doc.status==='failed'?doc.error:'Legacy job queued'}</p>{doc.concept_path&&<p>{doc.bundle_name} / {doc.concept_path}</p>}</div>
 <div className="knowledge-document-actions"><button onClick={()=>setSelected(doc.id)}>Preview & review</button>{admin&&<button disabled={busy} onClick={async()=>{if(!window.confirm(`Remove “${doc.name}”, its revisions and images? Saved answers remain.`))return;setBusy(true);try{await api(`/api/documents/${doc.id}`,undefined,'DELETE');await load();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>Remove</button>}</div></article>)}</div>
