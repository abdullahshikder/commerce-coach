...
export function providerUsage(provider:string,usage:{inputTokens?:unknown;outputTokens?:unknown;costUsd?:unknown}){
 const number=(value:unknown)=>typeof value==='number'&&Number.isFinite(value)&&value>=0?value:undefined;
 console.log(JSON.stringify({event:'provider_usage',provider,inputTokens:number(usage.inputTokens),outputTokens:number(usage.outputTokens),costUsd:number(usage.costUsd)}));
}
