...
try{await client.query('BEGIN');await client.query("SELECT pg_advisory_xact_lock(hashtext('commerce-coach-migrate'))");await client.query(await readFile(new URL('../server/migrations/007_production.sql',import.meta.url),'utf8'));await client.query('COMMIT');console.log('Production pipeline migration applied; no accounts or roles created.');}catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();await pool.end();}
